...
void ReportEnginePrivate::internalPrintPages(ReportPages pages, QPrinter& printer) {
    ...
    foreach (PageItemDesignIntf::Ptr page, pages) {
        if (!m_cancelPrinting && ((printer.printRange() == QPrinter::AllPages) || ((printer.printRange() == QPrinter::PageRange) && (currenPage >= printer.fromPage()) && (currenPage <= printer.toPage())))) {
...
bool ReportEnginePrivate::printPagesExcel(ReportPages pages, const QString& fileName, bool isSingleHeader) {
    ...
    try {
        ...
        if (pages.count() > 0) {
            ...
            foreach (PageItemDesignIntf::Ptr page, pages) {
                if (!m_cancelPrinting) {
                    excelHandler.exportPageToExcel(page, xlsx, startRow);
                    // qDebug() << "excelHandler.exportPageToExcel ok";
                    // emit pagePrintingFinished(currenPage);
                    QApplication::processEvents();
                }
                currenPage++;
...
bool ReportEnginePrivate::printToExcel(const QString& fileName) {
    return exportReport("Excel", fileName);
}
...
PrintProcessor::PrintProcessor(QPrinter* printer)
    : m_printer(printer)
    , m_painter(0)
    , m_firstPage(true) { m_renderPage.setItemMode(PrintMode); }
...
#else
    if (
        page->printBehavior() == PageItemDesignIntf::Split && m_printer->pageLayout().pageSize() != QPageSize((QPageSize::PageSizeId)page->pageSize()) && printerPageRect.width() < page->geometry().width()) {
#endif
